package com.example.harmonyapp.data.paging.remotemediator
...
import com.example.harmonyapp.data.model.PageKey
import com.example.harmonyapp.network.api.PicsumApi
import com.example.harmonyapp.utils.Constants.Companion.DEFAULT_PAGE_INDEX
...
class ImageListRemoteMediator(val service: PicsumApi, val db: ImageListDb,
                              private var imageLimit: MutableLiveData<Int>
) :
        ...
        try {
            ...
            val images =
                if(imageLimit.value==-1){
                    ...
                    val required = ((imageLimit.value?:0) - ((page-1) * QUERY_PER_PAGE))
                    resBody?.take(required)
                }
            var endOfPaginationReached = false
            images?.let {
                endOfPaginationReached = it.size<QUERY_PER_PAGE
            }
            db.withTransaction {
